...
const MAX_UNBIASED_BYTE =
  Math.floor(256 / BOOKING_CODE_ALPHABET.length) * BOOKING_CODE_ALPHABET.length;
...
export function generateBookingCode(): string {
  ...
  while (candidate.length < BOOKING_CODE_LENGTH) {
    const randomBytes = new Uint8Array(BOOKING_CODE_LENGTH - candidate.length);
    ...
    for (const byte of randomBytes) {
      ...
      candidate += BOOKING_CODE_ALPHABET.charAt(byte % BOOKING_CODE_ALPHABET.length);
