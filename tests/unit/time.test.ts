...
describe("Warsaw time helpers", () => {
  ...
  it("converts Warsaw local time to UTC in winter and summer", () => {
    expect(localDateTimeToUtc("2026-01-15", "10:00")).toBe(
      "2026-01-15T09:00:00.000Z",
    );
    expect(localDateTimeToUtc("2026-07-15", "10:00")).toBe(
  ...
  it("rejects the spring DST gap", () => {
    expect(() =>
      localDateTimeToUtc("2026-03-29", "02:30"),
    ).toThrow(RangeError);
  });

  it("rejects the autumn DST overlap instead of choosing an offset", () => {
    expect(() =>
      localDateTimeToUtc("2026-10-25", "02:30"),
    ).toThrow(RangeError);
  });
  ...
  it.each([15, 30, 45, 75])("adds %i exact UTC minutes", (minutes) => {
    expect(
      addMinutesToUtc("2026-07-15T08:00:00.000Z", minutes),
    ).toBe(
