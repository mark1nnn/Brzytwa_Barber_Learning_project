...
describe('positive integer IDs', () => {
  ...
  it.each([0, -1, 1.5, '0', '-1', '1.5', 'abc'])('rejects %s', (value) => {
    expect(positiveIntegerIdSchema.safeParse(value).success).toBe(false);
  });
});

describe('customer name', () => {
  it.each(['Jan Kowalski', 'Michał Nowak', 'Олександр Коваль', 'Anna-Maria Nowak', "O'Connor"])(
    'accepts %s',
    (value) => {
...
describe('Polish phone normalization', () => {
  ...
  it.each(['123', '+49123456789', 'abc123456789', '+++++++++', '1234567890'])(
    'rejects %s',
    (value) => {
...
describe('email and notes normalization', () => {
  it('trims and lowercases a valid email', () => {
    expect(customerEmailSchema.parse('  JAN@Example.COM ')).toBe('jan@example.com');
  });

  it('rejects invalid and oversized email values', () => {
    expect(customerEmailSchema.safeParse('not-an-email').success).toBe(false);
    expect(customerEmailSchema.safeParse(`${'a'.repeat(245)}@example.com`).success).toBe(false);
  });
  ...
  it('preserves notes as plain text', () => {
    expect(customerNotesSchema.parse('<b>bez zmian</b>')).toBe('<b>bez zmian</b>');
  });

  it('enforces the notes length limit', () => {
    expect(customerNotesSchema.parse('a'.repeat(500))).toHaveLength(500);
    expect(customerNotesSchema.safeParse('a'.repeat(501)).success).toBe(false);
  });
});
...
describe('privacy, dates, and timestamps', () => {
  ...
  it.each(['2026-02-28', '2028-02-29'])('accepts local date %s', (value) => {
    expect(localDateSchema.parse(value)).toBe(value);
  });

  it.each(['2026-02-29', '2026-02-30', '2026-13-01'])('rejects local date %s', (value) => {
    expect(localDateSchema.safeParse(value).success).toBe(false);
  });

  it('requires UTC timestamps with Z', () => {
    expect(utcTimestampSchema.parse('2026-07-15T08:00:00.000Z')).toBe('2026-07-15T08:00:00.000Z');
    expect(utcTimestampSchema.safeParse('2026-07-15T10:00:00').success).toBe(false);
  });
});
