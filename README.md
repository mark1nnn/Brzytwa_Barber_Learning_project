...
включая проверку целостности слотов записи.
...
Проверенная локальная среда на момент этапа 2.5: Node.js 24.13.0, npm 11.6.2 и
Wrangler 4.106.0.
...
Vitest и Playwright не подключены на этапах 1–2.5. Unit, integration и E2E тесты
...
`nodejs_compat` не включён: на этапе 2.5 он не требуется.
...
резервной копии. Этап 2.5 не запускает remote-команды автоматически.
...
этапе 2.5 намеренно не создаётся.
...
Этапы 1, 2 и подготовительный этап 2.5 завершены. Этап 3 ещё не реализован:
...
этапов 1, 2 и 2.5. `.gitattributes` предотвращает бессмысленные EOL-diff и не
